...
const char *Config::PowerRune_description = "PowerRune v1.0";
...
esp_err_t Config::read() {
    ...
    if (err != ESP_OK) {
        ...
        config_info.armour_id = CONFIG_ARMOUR_ID;
    ...
    nvs_close(nvs);
#if CONFIG_POWERRUNE_TYPE == 0
    ESP_LOGI(TAG, "Config loaded: armour_id=%d (CONFIG_ARMOUR_ID=%d)", config_info.armour_id, CONFIG_ARMOUR_ID);
#endif
    return ESP_OK;
}
...
void Config::global_event_handler(void *, esp_event_base_t, int32_t id, void *event_data) {
...
void Firmware::task_OTA(void *) {
    ESP_LOGW(TAG, "OTA stub");
    vTaskDelete(NULL);
}

void Firmware::global_system_event_handler(void *, esp_event_base_t, int32_t, void *) {}

void Firmware::global_pr_event_handler(void *, esp_event_base_t, int32_t id, void *) {
