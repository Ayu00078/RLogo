...
void Motor::send_motor_current(current_info_t &current_info)
{
    ...
    if (err != ESP_OK)
    {
        ESP_LOGW("TWAI", "twai_transmit failed: %s", esp_err_to_name(err));
        if (err == ESP_ERR_INVALID_STATE)
        {
            twai_stop();
            twai_start();
            // 清零电流，避免Bus-Off恢复后立刻发送旧的大电流值导致电机突然加速
...
Motor::Motor(uint8_t *motor_id, uint8_t motor_counts, gpio_num_t TX_TWAI_GPIO, gpio_num_t RX_TWAI_GPIO, float Kp, float Ki, float Kd, float Pmax, float Imax, float Dmax, float max) : PID(Kp, Ki, Kd, Pmax, Imax, Dmax, max)
{
    ...
    ESP_LOGI("TWAI", "PID params: Kp=%.2f Ki=%.2f Kd=%.2f Imax=%.1f Dmax=%.1f OutMax=%.1f",
             Kp, Ki, Kd, Imax, Dmax, max);
...
esp_err_t Motor::unlock_motor(uint8_t motor_id)
{
    ...
    for (size_t i = 0; i < motor_counts; i++)
    {
        if (motor_info[i].motor_id == motor_id)
        {
            ...
            motor_info[i].last_received = xTaskGetTickCount();
            // 解锁前重置PID，清除断线期间的积分残留，防止启动时电流冲击
            reset();
            ESP_LOGI(TAG_TWAI, "Motor %d unlocked, PID reset.", motor_id);
...
{
    float abs_amp=fabsf(amplitude);
    abs_amp=fmaxf(0.780f,fminf(1.045f,abs_amp));
    amplitude=(amplitude>=0.0?1.0f:-1.0f)*abs_amp;
    omega = fmaxf(1.884f, fminf(2.000f, omega));
    float enforced_offset=(amplitude>=0.0?1.0f:-1.0f)*(2.090f-abs_amp);
    for (size_t i = 0; i < motor_counts; i++)
    {
        if (motor_info[i].motor_id == motor_id)
        {
            ...
            motor_info[i].speed_trace_sin_info.offset = enforced_offset;
...
motor_status_t Motor::get_motor_status(uint8_t motor_id)
{
    ...
    {
        if (motor_info[i].motor_id == motor_id)
            return motor_info[i].motor_status;
    }
...
void Motor::state_check()
{
    ...
    // receive队列是否为空
    if (twai_receive(&twai_msg_re, pdMS_TO_TICKS(10)) == ESP_OK)
    {
        // 获取电机ID
        ...
        for (size_t i = 0; i < motor_counts; i++)
        {
            if (motor_info[i].motor_id == motor_id)
            {
                ...
                {
                    motor_info[i].motor_status = MOTOR_DISABLED_LOCKED;
                    ESP_LOGI(TAG_TWAI, "Motor %d reconnected.", motor_id);
                }
    ...
    while (1)
    {
        ...
        for (size_t i = 0; i < motor_counts; i++)
        {
            ...
            case MOTOR_NORMAL_PENDING:
                ...
                {
                    static uint32_t last_dbg_n = 0;
                    if (xTaskGetTickCount() - last_dbg_n > 20)
                    {
                        printf(">set_spd:%d\n>actual:%d\n>delta:%.1d\n>iq1:%d\n",
                            motor_info[i].set_speed, motor_info[i].speed, motor_info[i].set_speed - motor_info[i].speed, current_info.iq1);
                        last_dbg_n = xTaskGetTickCount();
                    }
                }
                break;
            ...
            case MOTOR_TRACE_SIN_STABLE:
                ...
                set_current(motor_info[i].motor_id, PID_CURRENT, current_info);
                {
                    ...
                    if (xTaskGetTickCount() - last_dbg > 20)
                    {
                        printf(">set_spd:%d\n>actual:%d\n>delta:%.1d\n>iq1:%d\n",
                            motor_info[i].set_speed, motor_info[i].speed, motor_info[i].set_speed - motor_info[i].speed, current_info.iq1);
